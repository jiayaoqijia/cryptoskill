...
```bash
tmp="$(mktemp -d)"
git clone --depth 1 https://github.com/Vo1ganin/solana-rpc-skill.git "$tmp/repo"
rm -rf "$HOME/.claude/skills/solana-rpc"
mkdir -p "$HOME/.claude/skills"
cp -R "$tmp/repo" "$HOME/.claude/skills/solana-rpc"
rm -rf "$HOME/.claude/skills/solana-rpc/.git" "$tmp"
```
