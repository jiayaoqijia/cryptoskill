...
Live chain data: `https://8004scan.io/api/v1/public/chains`
...
## Current Chain Counts

Agent counts change frequently. Query the live API instead of relying on a snapshot:

```bash
curl -s "https://8004scan.io/api/v1/public/chains" | jq '.data.chains // .data.data.chains // .data'
curl -s "https://8004scan.io/api/v1/public/agents?chainId=8453&limit=1&page=1" | jq '.meta.pagination.total'
```
