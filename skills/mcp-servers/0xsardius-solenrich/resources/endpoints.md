...
| `stonk-alerts` | $0.02 | `mints` (1–25), `since` (`min_holders_change_pct` 10, `stale_after_hours` 24) | Since your last check: `payout_landed`, `payout_stale`, `stopped_trading`, `holders_change`, `rewards_since`, plus payout status per coin; pass `checked_at` back as `since` |
