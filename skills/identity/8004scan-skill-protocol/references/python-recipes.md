...
def get_owner_agents(address: str) -> list:
    ...
    resp = requests.get(
        f"https://8004scan.io/api/v1/public/accounts/{address}/agents"
    )
    data = resp.json()
...
def poll_new_agents(chain_id: int, interval: int = 60):
    ...
    while True:
        resp = requests.get(
            ...
            params={"chainId": chain_id, "limit": 20, "page": 1, "sortBy": "created_at", "sortOrder": "desc"}
        )
        if resp.ok:
            for agent in resp.json()["data"]:
                aid = f"{agent['chain_id']}:{agent['token_id']}"
                if aid not in seen:
                    seen.add(aid)
                    yield agent
        time.sleep(interval)
