...
console.log(`Services: ${metadata.services?.map((svc) => svc.name).join(", ") ?? "none"}`);
```
...
const resp = await fetch(
  `https://8004scan.io/api/v1/public/agents/${agent.chainId}/42`,
  { headers: { "X-API-Key": process.env.EIGHTSCAN_API_KEY ?? "" } }
);
const enriched = await resp.json();
console.log(`Score: ${enriched.data.total_score}`);
console.log(`Feedbacks: ${enriched.data.total_feedbacks}`);
```
