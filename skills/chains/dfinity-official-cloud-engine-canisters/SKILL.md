...
- **Management-canister calls keep their standard wrapper.** `Call.httpRequest` (Motoko, `mo:ic`) and the Rust outcall wrapper for your crate line (Rule 3) ask the `ic0.cost_*` system API for the fee, and that API is cost-schedule aware: for an outcall it prices against **your own** subnet's cost schedule, which is free on an engine, so it returns **0**, the wrapper attaches nothing, and the same source stays portable to an Application subnet. What breaks is substituting a hardcoded or hand-computed fee — never take a number from the `https-outcalls` cost tables and attach it on an engine.
- **Which subnet's schedule is consulted is the whole distinction.** The chain-key cost APIs (`ic0.cost_sign_with_ecdsa` and friends) price against the subnet that **holds the key**, not yours — that subnet charges normally, so they return a non-zero fee even when called from an engine, and the wrapper then attaches it. That is why a helper that signs cannot work here while the same crate's outcall builder can (Rules 3 and 4).
...
- **Keep the wrapper; never swap in a hardcoded fee** (Rule 1). Letting the wrapper price the call is how outcalls are written in the Motoko engine apps running today.
- **In Rust, use the wrapper your crate line provides** — `HttpRequest::new(url)…send()` from `ic-cdk-management-canister` 0.2, the free `ic_cdk_management_canister::http_request` in 0.1, or `ic_cdk::management_canister::http_request` on ic-cdk 0.19 (that module was **removed in ic-cdk 0.20**). All three behave identically here: each asks `ic0.cost_*`, which prices against your own subnet and so returns 0 on an engine, and each attaches that 0. Write for the line the project is actually on, and never bump crate versions to "fix" an engine outcall — there is nothing to fix. `https-outcalls` covers the differences between the three.
- **`ic-cdk-management-canister`'s outcall builders are the supported path here** — only its *chain-key* helpers are unusable on an engine (Rule 4). The crate is not off-limits; the target subnet is what decides.
- **Pricing version 2 needs no tuning on an engine.** The 0.2 builder always prices version 2 ("pay-as-you-go"), where the cycles attached double as each node's spending budget — so on a paying subnet an under-narrowed `with_expected_*` lets a call fail partway, after the remote server was contacted. On an engine that cannot happen: the free cost schedule never charges, so the budget never narrows anything — the round-trip timeout and the transform-instruction limit sit at the protocol maximum, and `InsufficientCycles` is never raised. Treat the `with_expected_*` setters as no-ops here: set them for portability if you like, but never tune them to fix an engine outcall, and never read an engine outcall failure as a budget problem. What still applies is your **own** `max_response_bytes` — it bounds the response on an engine exactly as anywhere else, so an oversized reply is a real failure cause and the one outcall limit worth checking here.
- **Either pricing version works on an engine, so the version is not something to manage here.** Version 2 is accepted only where the replica has pay-as-you-go enabled; where it does not, a requested version 2 is **silently filtered back to version 1** with no error. That is a funding mismatch on a paying subnet, but harmless on an engine, because an outcall is charged zero under *both* versions. Do not add engine-specific version handling.
...
**The chain-key helpers do not work here.** `ic-vetkeys`, `ic-cdk-management-canister`'s chain-key functions (`sign_with_ecdsa`, `sign_with_schnorr`, `vetkd_derive_key` and their public-key siblings), and Motoko's `mo:ic-vetkeys/ManagementCanister` all call `aaaaa-aa` directly and attach the fee for you. Because the chain-key cost API prices against the key-holding subnet, that fee is non-zero even on an engine, and the helper issues an unbounded-wait call carrying it — a cross-subnet, cycle-bearing call, rejected twice over (Rules 1 and 2). Their convenience is exactly what you must give up: encode the management-canister argument yourself and send it through the proxy. The `vetkeys` skill's code is written for a normal subnet and needs this rewrite before it will run on an engine.

**This is scoped to the chain-key functions, not to the crates.** `ic-cdk-management-canister` also carries the HTTPS-outcall builders, and those are the supported way to make an outcall from an engine canister (Rule 3): they target your own subnet, price at 0, and must **not** be routed through the proxy. Reading "the helpers don't work on an engine" as "hand-encode outcalls through the proxy too" produces exactly the consensus and funding failures in pitfall 4. Split the crate by target subnet: local outcall ⇒ ordinary builder; remote chain-key ⇒ proxy.
...
3. **Rewriting outcall code for an engine — or hardcoding its fee.** Outcalls need no engine-specific form: keep `Call.httpRequest` (Motoko) or, in Rust, whichever wrapper the project's crate line provides (`HttpRequest::new(url)…send()` on `ic-cdk-management-canister` 0.2; `ic_cdk::management_canister::http_request` only up to ic-cdk 0.19, since 0.20 removed that module). The `ic0.cost_*` API they consult prices against your own subnet and returns 0 on an engine. The mistake is replacing that call with a hand-computed fee from an Application-subnet cost table, which attaches a non-zero amount against a 0 balance.
...
10. **Using a chain-key helper library on an engine.** `ic-vetkeys`, `ic-cdk-management-canister`'s chain-key functions, and `mo:ic-vetkeys/ManagementCanister` call `aaaaa-aa` directly and attach the fee themselves — non-zero even on an engine, because the chain-key cost API prices against the key-holding subnet — making it a cross-subnet, cycle-bearing call that the engine rejects under Rules 1 and 2. Their whole value proposition (correct types, automatic cycles) is what has to go: hand-encode the management-canister argument and relay it through the proxy. Code copied from the `vetkeys` skill will not run on an engine unmodified.
11. **Over-reading that warning and proxying outcalls too.** The warning above is about the chain-key functions, not the crates that contain them. `ic-cdk-management-canister` is also what Rust outcall code should use (Rule 3), and its `HttpRequest` builder is correct on an engine as written. Concluding "this crate doesn't work on an engine, so the outcall must be hand-encoded through the proxy" lands straight in pitfall 4 — a transform that cannot apply and a proxy balance that drains. Decide by target subnet, not by crate name: `aaaaa-aa` `http_request` is local and free; signing and vetKD are remote and paid.
12. **Tuning pricing-version-2 budgets for an engine.** `ic-cdk-management-canister` 0.2 always prices version 2, where the attached cycles are also each node's budget and a too-narrow `with_expected_*` can fail a call partway. On an engine the free cost schedule never charges, so the budget narrows nothing: the timeout and transform-instruction limits sit at the protocol maximum and `InsufficientCycles` cannot be raised. The setters therefore change nothing here — do not add them to fix an engine outcall, and do not diagnose an engine outcall failure as an exhausted budget. Your own `max_response_bytes` is the exception: it still bounds the response, so an oversized reply remains a genuine cause. (Nor does the pricing version itself need managing: where the replica lacks pay-as-you-go, version 2 is silently filtered to version 1, and both cost zero on an engine.)
13. **Deleting or swapping the proxy of a signing app.** The management canister derives keys from *its* caller (the proxy), so the proxy's canister id is part of every address the app owns. Deleting a proxy (the console refunds its cycles) or repointing the app at another one silently produces different addresses and leaves any Bitcoin or Ethereum funds at the old ones permanently unspendable. A signing app's proxy id is permanent infrastructure, not a fungible resource.
14. **Reading `CallFailed` as "it did not happen".** The proxy relays with a bounded-wait call, so a downstream `SYS_UNKNOWN` arrives as `ProxyError::CallFailed` with that reject code in `reason`. The target may have run and the cycles may be gone. Retrying a signature is harmless; retrying a stateful call on this basis is not: treat it as an unknown outcome (see `multi-canister`).
15. **Calling the console proxy from the CLI.** `icp canister call --proxy <console-proxy-id>` fails: the console proxy admits only the engine's canister-id ranges and its own controllers, and an ingress call from your CLI identity is neither (it is rejected by `inspect_message` before it reaches replicated state). For CLI-side proxying, deploy your own proxy (`icp new … --subfolder proxy`) which authorizes controllers, i.e. you.
