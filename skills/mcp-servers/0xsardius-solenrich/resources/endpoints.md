...
| `stonk-yield-batch` | $0.05 | none (`mints` up to 25, or the screener filters; `limit` ≤ 25) | `stonk-yield` for up to 25 coins in one call, from the index in milliseconds; `not_found` for mints outside it |
| `stonk-alerts` | $0.005 | `mints` (1–25), `since` (`min_holders_change_pct` 10, `stale_after_hours` 24) | Since your last check: `payout_landed`, `payout_stale`, `stopped_trading`, `holders_change`, `rewards_since`, plus payout status per coin; pass `checked_at` back as `since` |
