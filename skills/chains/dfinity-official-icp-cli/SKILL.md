...
13. **Porting the `@dfinity/agent` actor setup to `@icp-sdk/bindgen`.** The old `@dfinity/agent` pattern was `createActor(canisterId, { agent })` with `new HttpAgent({ identity, host })`. The `@icp-sdk/bindgen` pattern is `createActor(canisterId, { agentOptions: { identity, rootKey } })` — the binding creates the agent internally, and `host` stays unset in browser code. A pre-built `{ agent }` still works, but `new HttpAgent()` is deprecated. See `references/binding-generation.md` for the correct pattern and for when `host` must be set.
