...
describe('wallet context storage', () => {
  ...
  beforeEach(() => {
    tempDir = join(tmpdir(), `tomorrow-wallet-context-${Date.now()}-${Math.random()}`);
  ...
  test('setActiveWalletProfile creates directory and context file', () => {
    ...
    expect(readWalletContext()?.profiles.default?.address).toBe('ELF_tomorrow_AELF');
  });
