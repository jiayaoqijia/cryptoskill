---
name: chainstack-official-rpc-nodes-mcp
description: "This repository is deprecated. All MCP functionality is now available through the unified **Chainstack MCP server** at [`mcp.chainstack.com`](https://mcp.chainstack.com)."
---

# ⚠️ Deprecated — use the Chainstack MCP server

This repository is deprecated. All MCP functionality is now available through the unified **Chainstack MCP server** at [`mcp.chainstack.com`](https://mcp.chainstack.com).

## Quick start

Type this into any AI agent:

```
get mcp.chainstack.com
```

The agent will set everything up for you — node deployment, docs search, platform status, and more. No local setup required.

## Learn more

- [Chainstack MCP server docs](https://docs.chainstack.com/docs/chainstack-mcp-server)
- [Discovery page](https://mcp.chainstack.com/) — agent-readable onboarding for all major agents
- [Skill file](https://mcp.chainstack.com/skill) — for on-demand usage without MCP registration
