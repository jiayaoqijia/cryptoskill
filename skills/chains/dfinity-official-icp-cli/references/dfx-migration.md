...
**`createActor` takes agent options.** The `@dfinity/agent` pattern builds an agent and passes `{ agent }`. With `@icp-sdk/bindgen`, pass `{ agentOptions }` and the binding builds the agent (a pre-built `{ agent }` still works and is used as-is). When calling the network that serves the page, set `rootKey` from the `ic_env` cookie — without it the agent defaults to the mainnet root key and local calls fail verification (for a page calling a different network, see `references/binding-generation.md`). Carry the `identity` over into `agentOptions`: without it, calls are anonymous. Do not carry `new HttpAgent()` over: the constructor is deprecated in `@icp-sdk/core`; use `await HttpAgent.create()` if you need your own agent.
...
createActor(canisterEnv?.["PUBLIC_CANISTER_ID:backend"], {
  ...
  // Keep the identity from the old agent; without it, calls are anonymous.
